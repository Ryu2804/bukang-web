...
def get_student_by_nrp(db: Session, nrp: str) -> Student:
    student = db.query(Student).filter(Student.nrp == nrp).first()
    if not student:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Student not found")
    return student


def search_students_by_nrp(db: Session, nrp: str) -> list[Student]:
    return db.query(Student).filter(Student.nrp.like(f"%{nrp}%")).order_by(Student.nrp).all()


def create_student(db: Session, data: StudentCreate) -> Student:
    existing = db.query(Student).filter(Student.nrp == data.nrp).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="nrp already exists",
        )
    ...
    if data.nrp[2] == "2" and data.nrp[3] == "5":
        ...
        major = "Unknown"
    student = Student(nrp=data.nrp, name=name, major=major)
    db.add(student)
    db.commit()
    db.refresh(student)
    return student
